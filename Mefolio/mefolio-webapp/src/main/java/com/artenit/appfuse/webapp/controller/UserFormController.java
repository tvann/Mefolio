...
import com.artenit.appfuse.webapp.util.RequestUtil;
...
public class UserFormController extends BaseFormController {
    ...
    @RequestMapping("/userform?method=Delete*")
    ...
    public String onSubmit(User user, BindingResult errors, HttpServletRequest request,
        ...
        if (request.getParameter("delete") != null) {
            ...
            try {
                ...
                //not keep the confirmPassword in clear data - VNT -28/02/2013
            ...
            //When the admin or user saves its profile
            if (!StringUtils.equals(request.getParameter("from"), "list")) {
                saveMessage(request, getText("user.saved", user.getFullName(), locale));
                // return to main Menu
                return getCancelView();
            } else {//When the admin add a new user
                if (StringUtils.isBlank(request.getParameter("version"))) {
                    ...
                    return getSuccessView();
                } else {//When the admin update a user profile
